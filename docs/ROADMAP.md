...
Before the Architect plans, the PM writes a `spec.md`. This is the **contract** — every agent's work is measured against it.
...
Enforcement: PM writes → user approves → Architect references in plan.md → QA validates acceptance criteria → PM verifies at completion.
